...
def print_death_age(people_dict):
    ...
    # find the age of the individual by subracting
    # the death birth year from the death year

    print ('Ages at death:')
    
    for person, age in people_dict.items():
        name = age[NAME_INDEX]
        age = age[DEATH_YEAR_INDEX] - age[BIRTH_YEAR_INDEX]
...
def count_genders(people_dict):
    ...
    # Tally up the genders
    print ('Genders')
    
    # Females
    females = 0
    
    for person, ladies in people_dict.items():
        gender = ladies[GENDER_INDEX]
        if gender == 'F':
            females += 1
    
    # Males  
    people = len(people_dict)
    males = people - females
...
def print_marriages(marriages_dict, people_dict):
    ...
    for couple, duration in marriages_dict.items():
        husband = duration[HUSBAND_KEY_INDEX]
        wife = duration[WIFE_KEY_INDEX]
        year_of_wedding = duration[WEDDING_YEAR_INDEX]
        
        husbands = people_dict[husband]
        husband_name = husbands[NAME_INDEX]
        birth_of_husband = husbands[BIRTH_YEAR_INDEX]
        
        age_of_husband = year_of_wedding - birth_of_husband
        
        wives = people_dict[wife]
        wife_name = wives[NAME_INDEX]
        birth_of_wife = wives[BIRTH_YEAR_INDEX]
        
        age_of_wife = year_of_wedding - birth_of_wife
        
        print(f"{husband_name} {age_of_husband}" \
            f" > {year_of_wedding} < {wife_name} {age_of_wife}")
