# value = dictionary_name[key] <- Find 1 key and its corresponding value in a dictionary
# dictionary_name[key] = value <- Adding an item to an existing dctionary
# if key in dictionary_name: <- Check to see if a key is stored in a dictionary
...
def main():
    ...
    if vin in vehicles_dict:
        ...
        vehicle_manu = vehicle[MANUFACTURER_INDEX]
        vehicle_model = vehicle[MODEL_INDEX]
        vehicle_colour = vehicle[COLOR_INDEX]
        print (f'Selected Vehicle: {vehicle_manu} {vehicle_model} {vehicle_colour}')
